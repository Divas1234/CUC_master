...
function readxlssheet()
    ...
    df = XLSX.readxlsx("/home/yuanyiping/下载/task 9/master-5/inputdata/data2_for168h.xlsx") # for little case
...
function forminputdata(DataGen, DataBranch, DataLoad, LoadCurve, GenCost, UnitsFreqParam, StrogeData)
    ...
    Loads_SumLoad = LoadCurve[:, 2] / 100 * 0.8
    Loads_PerLoad = zeros(ND, NT)
    for i in 1:NT
        Loads_PerLoad[:, i] = Loads_SumLoad[i, 1] .* Loads_Percent[:, 1] * 1.5
    end
