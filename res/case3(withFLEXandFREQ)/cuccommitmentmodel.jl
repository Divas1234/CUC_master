...
function refined_cscucmodel(
    ...
    set_optimizer_attribute(scuc, "MIPGap", 0.05)
    ...
    for s in 1:NS
        ...
        #     sum((time_forflexcheck[t] == 1) ? cunits.p_0[:, 1] : each_pg₀[(1+(s-1)*clustered_NG):(s*clustered_NG), time_forflexcheck[t]-1]) >= flexbility_limit
        # )
        # @constraint(
        #     scuc,
        #     sum(each_pg₀[(1+(s-1)*clustered_NG):(s*clustered_NG), time_forflexcheck[t]]) -
        #     [t = 1:new_NT],
        #     sum((time_forflexcheck[t] == 1) ? cunits.p_0[:, 1] : each_pg₀[(1+(s-1)*clustered_NG):(s*clustered_NG), time_forflexcheck[t]-1]) >=
        #     sum(loads.load_curve[:, time_forflexcheck[t]] .- ((time_forflexcheck[t] == 1) ? loads.load_curve[:, time_forflexcheck[t]] * 0.75 : loads.load_curve[:, time_forflexcheck[t]-1])) * 0.100
        # )
        @constraint(
            scuc,
            [t = 2:new_NT],
    ...
    open("/home/yuanyiping/下载/task 9/master-5/res/pro_calculation_result.txt", "w") do io
