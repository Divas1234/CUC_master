...
function simfilied_cscucmodel(
    ...
    for s in 1:NS
        ...
        #     sum((time_forflexcheck[t] == 1) ? cunits.p_0[:, 1] : pg₀[(1+(s-1)*clustered_NG):(s*clustered_NG), time_forflexcheck[t]-1]) >= flexbility_limit
        # )
        # @constraint(
        #     scuc,
        #     [t = 1:new_NT],
        #     sum(pg₀[(1+(s-1)*clustered_NG):(s*clustered_NG), time_forflexcheck[t]]) -
        #     sum((time_forflexcheck[t] == 1) ? cunits.p_0[:, 1] : pg₀[(1+(s-1)*clustered_NG):(s*clustered_NG), time_forflexcheck[t]-1]) >=
        ...
        @constraint(
            ...
            sum(cunits.shut_up[:, 1] .* ((time_forflexcheck[t] == 1) ? ones(NG, 1) : (x[:, time_forflexcheck[t]-1] - x[:, time_forflexcheck[t]-1]))) +
            sum(cunits.p_max[:, 1] .* (ones(NG, 1) - ((time_forflexcheck[t] == 1) ? onoffinit[:, 1] : x[:, time_forflexcheck[t]-1]))) >=
        ...
        @constraint(
            scuc,
            [t = 2:new_NT],
            sum(cunits.p_max[:, 1] .* ((time_forflexcheck[t] == 1) ? onoffinit[:, 1] : x[:, time_forflexcheck[t]-1])) -
            sum(pg₀[(1+(s-1)*clustered_NG):(s*clustered_NG), time_forflexcheck[t]]) >=
            sum(loads.load_curve[:, time_forflexcheck[t]] .- ((time_forflexcheck[t] == 1) ? loads.load_curve[:, time_forflexcheck[t]] * 0.75 : loads.load_curve[:, time_forflexcheck[t]-1])) * 1.00
        )
    end
    ...
    open("/home/yuanyiping/下载/task 9/master-5/res/simpro_calculation_result.txt", "w") do io
        writedlm(io, [" "])
