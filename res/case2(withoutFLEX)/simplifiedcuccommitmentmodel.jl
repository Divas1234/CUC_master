...
function simfilied_cscucmodel(
    ...
    @constraint(
        ...
        ((t == 1) ? units.p_0[:, 1] : pg₀[(1+(s-1)*clustered_NG):(s*clustered_NG), t-1]) .<=
    ...
    for s in 1:NS
        time_forflexcheck = findall(x -> x == 1, rampingup_critical_scenario[s, 1])
        new_NT = size(time_forflexcheck, 1)
        @constraint(
            scuc,
            [t = 1:new_NT],
            sum(each_pg₀[(1+(s-1)*NG):(s*NG), time_forflexcheck[t]]) -
            sum((time_forflexcheck[t] == 1) ? units.p_0[:, 1] : each_pg₀[(1+(s-1)*NG):(s*NG), time_forflexcheck[t]-1]) >= flexbility_limit
        )
    end
    ...
    #     scuc,
    #     [t = 1:NT, i = 1],
    #     ζ[i, t] <= sum(units.p_max[:, 1] .* z[((i - 1) * clustered_NG + 1):(i * clustered_NG), t])
    # )
