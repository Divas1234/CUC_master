...
function recognizing_critical_scenarios(winds, loads, NT)
	loadcurve = zeros(1, size(loads.load_curve, 2))
	for t ∈ 1:size(loads.load_curve, 2)
	...
	return FLEX_indified_criticalscenarios, FREQ_indified_criticalscenarios
end
