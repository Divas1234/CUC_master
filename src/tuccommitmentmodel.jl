...
function scucmodel(NT::Int64,
	...
	#         sum((time_forflexcheck[t] == 1) ? units.p_0[:, 1] : pg₀[(1+(s-1)*NG):(s*NG), time_forflexcheck[t]-1]) >= flexbility_limit
	#     )
	# end
	for s in 1:NS
		...
		#     [t = 1:new_NT],
		#     sum(pg₀[(1+(s-1)*NG):(s*NG), time_forflexcheck[t]]) -
		#     sum((time_forflexcheck[t] == 1) ? units.p_0[:, 1] : pg₀[(1+(s-1)*NG):(s*NG), time_forflexcheck[t]-1]) >=
	...
	prod_cost = pₛ *
				c₀ *
				(sum(sum(sum(sum(pᵪ[i + (s - 1) * NG, t, :] .* eachslope[:, i] for t in 1:NT)) for
						 s in 1:NS) for i in 1:NG) + sum(sum(sum(x₀[:, t] .* refcost[:, 1] for t in 1:NT)) for s in 1:NS))
	cr⁺ = pₛ *
		  c₀ *
