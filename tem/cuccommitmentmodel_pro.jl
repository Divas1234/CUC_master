...
include("/home/yuanyiping/下载/task 9/master-5/src/linearization.jl")
include("/home/yuanyiping/下载/task 9/master-5/src/powerflowcalculation.jl")
...
function refined_cscucmodel_withFreqandFlex(
    ...
    for s in 1:NS
        ...
        @constraint(
            ...
            sum(loads.load_curve[:, time_forflexcheck[t]] .- ((time_forflexcheck[t] == 1) ? loads.load_curve[:, time_forflexcheck[t]] * 1.0 : loads.load_curve[:, time_forflexcheck[t]-1])) * 1.00
        )
        @constraint(
            scuc,
            [t = 1:new_NT],
            sum(units.p_max[:, 1] .* ((time_forflexcheck[t] == 1) ? onoffinit[:, 1] : z[:, time_forflexcheck[t]-1])) -
            sum(each_pg₀[(1+(s-1)*NG):(s*NG), time_forflexcheck[t]-1]) >=
            sum(loads.load_curve[:, time_forflexcheck[t]] .- ((time_forflexcheck[t] == 1) ? loads.load_curve[:, time_forflexcheck[t]] * 1.0 : loads.load_curve[:, time_forflexcheck[t]-1])) * 1.00
        )
        @constraint(
            scuc,
            [t = 2:24],
            sum(units.ramp_up[:, 1] .* z[:, t-1]) +
            sum(units.shut_up[:, 1] .* (z[:, t] - z[:, t-1])) +
            sum(units.p_max[:, 1] .* (ones(NG, 1) - z[:, t-1])) >= 50
        )
        @constraint(
            scuc,
            [t = 2:24],
            sum(units.p_max[:, 1] .* z[:, t-1]) - sum(each_pg₀[(1+(s-1)*NG):(s*NG), t]) >= 50
        )
    end
    ...
    @constraint(
        ...
        >=
        Δp * f_base / RoCoF_max / 50 * 1.0 * 0.85
    )
    ...
    open("/home/yuanyiping/下载/task 9/master-5/tem/PRO_calculation_result_withFreqandFlex.txt", "w") do io
        ...
        return writedlm(io, JuMP.value.(pg₀[1:clustered_NG, 1:NT]), '\t')
    end
